...
/**
 * StaticLayoutLike is a Layout for text that will not be edited after it
 ...
 */
public class StaticLayoutLike extends Layout {

    static final String TAG = "StaticLayoutLike";

    /**
     * Builder for static layouts. The builder is a newer pattern for constructing
     * StaticLayoutLike objects and should be preferred over the constructors,
    ...
    public final static class Builder {
        ...
        /**
         * Obtain a builder for constructing StaticLayoutLike objects
         *
         ...
         * @return a builder object used for constructing the StaticLayoutLike
         */
        public static Builder obtain(CharSequence source, int start, int end, TextPaint paint,
            ...
            b.mMeasuredText = MeasuredTextLike.obtain();
            return b;
        }
        ...
        private static void recycle(Builder b) {
            b.mPaint = null;
            b.mText = null;
            MeasuredTextLike.recycle(b.mMeasuredText);
         ...
         * of normal {@link StaticLayoutLike} usage.
         *
         * @param source The text to be laid out, optionally with spans
         * @param start  The index of the start of the text
         * @param end    The index + 1 of the end of the text
         ...
         *
         * @param alignment Alignment for the resulting {@link StaticLayoutLike}
         * @return this builder, useful for chaining
         */
         ...
         *
         * @return the newly constructed {@link StaticLayoutLike} object
         */
        public StaticLayoutLike build() {
            StaticLayoutLike result = new StaticLayoutLike(this);
        ...
        MeasuredTextLike mMeasuredText;
    ...
    public StaticLayoutLike(CharSequence source, TextPaint paint,
                            int width,
    ...
    public StaticLayoutLike(CharSequence source, int bufstart, int bufend,
...
//            e.mMethod = b.mEllipsize;
    ...
    StaticLayoutLike(CharSequence text) {
    ...
    private StaticLayoutLike(Builder b) {
...
//        StaticLayout.LineBreaks lineBreaks = new StaticLayout.LineBreaks();  // TODO: move to builder to avoid allocation costs
        Object lineBreaks = LineBreaksProxy.getObjectFromInnerClass();  // TODO: move to builder to avoid allocation costs
        ...
        MeasuredTextLike measured = b.mMeasuredText;
        ...
        for (int paraStart = bufStart; paraStart <= bufEnd; paraStart = paraEnd) {
            ...
            if (spanned != null) {
                LeadingMarginSpan[] sp = LayoutProxy.getParagraphSpans(spanned, paraStart, paraEnd,
                ...
                chooseHt = LayoutProxy.getParagraphSpans(spanned, paraStart, paraEnd, LineHeightSpan.class);

                if (chooseHt == null || chooseHt.length == 0) {
            ...
            if (spanned != null) {
                TabStopSpan[] spans = LayoutProxy.getParagraphSpans(spanned, paraStart,
...
//            int[] flags = lineBreaks.flags;
            ...
            int[] breaks = LineBreaksProxy.breaks(lineBreaks);
            float[] lineWidths = LineBreaksProxy.widths(lineBreaks);
            int[] flags = LineBreaksProxy.flags(lineBreaks);
            int breakCount = nComputeLineBreaks(b.mNativePtr, lineBreaks, breaks,
                    lineWidths, flags, breaks.length);
    ...
    private int out(CharSequence text, int start, int end,
        ...
        Directions linedirs = LayoutProxy.getDIRS_ALL_LEFT_TO_RIGHT();
        ...
        if (easy) {
            ...
            mLineDirections[j] = AndroidBidiProxy.directions(dir, chdirs, start - widthStart, chs,
    ...
    private static long nNewBuilder() {
        return NativeProxy.nNewBuilder();
    }

    private static void nFreeBuilder(long nativePtr) {
        NativeProxy.nFreeBuilder(nativePtr);
    }

    private static void nFinishBuilder(long nativePtr) {
        NativeProxy.nFinishBuilder(nativePtr);
    }

//    /* package */ static native long nLoadHyphenator(ByteBuffer buf, int offset);
//

    private static void nSetLocale(long nativePtr, String locale, long nativeHyphenator) {
        NativeProxy.nSetLocale(nativePtr, locale, nativeHyphenator);
    }

    //
    private static void nSetIndents(long nativePtr, int[] indents) {
        NativeProxy.nSetIndents(nativePtr, indents);
    }

    // Set up paragraph text and settings; done as one big method to minimize jni crossings
    private static void nSetupParagraph(long nativePtr, char[] text, int length,
                                        float firstWidth, int firstWidthLineCount, float restWidth,
                                        int[] variableTabStops, int defaultTabStop, int breakStrategy,
                                        int hyphenationFrequency) {
        NativeProxy.nSetupParagraph(nativePtr, text, length, firstWidth, firstWidthLineCount,
                restWidth, variableTabStops, defaultTabStop, breakStrategy, hyphenationFrequency);
    }

    private static float nAddStyleRun(long nativePtr, long nativePaint,
                                      long nativeTypeface, int start, int end, boolean isRtl) {
        return NativeProxy.nAddStyleRun(nativePtr, nativePaint, nativeTypeface, start, end, isRtl);
    }

    private static void nAddMeasuredRun(long nativePtr,
                                        int start, int end, float[] widths) {
        NativeProxy.nAddMeasuredRun(nativePtr, start, end, widths);
    }

    private static void nAddReplacementRun(long nativePtr, int start, int end, float width) {
        NativeProxy.nAddReplacementRun(nativePtr, start, end, width);
    }

    private static void nGetWidths(long nativePtr, float[] widths) {
        NativeProxy.nGetWidths(nativePtr, widths);
    }
    ...
    private static int nComputeLineBreaks(long nativePtr, Object recycle,
                                          int[] recycleBreaks, float[] recycleWidths, int[] recycleFlags,
                                          int recycleLength) {
        return NativeProxy.nComputeLineBreaks(nativePtr, recycle, recycleBreaks, recycleWidths,
